<?php
// FILE: pages/sarana/hapus.php
// Proses Hapus Sarana Prasarana
...
if ($id_sarana == 0) {
    set_alert('danger', 'ID sarana tidak valid!');
...
if (!empty($sarana['foto'])) {
    $file_path = '../../uploads/foto_sarana/' . $sarana['foto'];
...
// Hapus dari database
$query = "DELETE FROM sarana_prasarana WHERE id_sarana = $id_sarana";

if (mysqli_query($koneksi, $query)) {
    set_alert('success', 'Sarana berhasil dihapus!');
} else {
    set_alert('danger', 'Gagal menghapus sarana: ' . mysqli_error($koneksi));
}
